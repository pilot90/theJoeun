		        ...
		        <table class="table table-striped rable-hover">
		            ...
		            <tbody>
		                <c:forEach var="result" items="${list}" varStatus="status">
		                    <tr>
		                        <td><c:out value="${paging.totalCnt - ((paging.pdto.page - 1) * paging.pdto.perPage) - status.index}"/></td>
		                        <td>
		                        	<c:forEach begin="1" end="${result.level}">&nbsp;&nbsp;</c:forEach>
		                        	<c:if test="${result.seq gt 0 }"><small class="label label-danger">RE:</small>&nbsp;</c:if>
		                        	<a href='view?code=${result.code}&page=${paging.pdto.page}&perPage=${paging.pdto.perPage}'><c:out value="${result.title}"/></a>
		                        </td>
